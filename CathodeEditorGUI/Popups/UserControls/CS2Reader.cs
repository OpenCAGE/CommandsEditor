...
namespace CathodeEditorGUI.Popups.UserControls
{
    public class CS2Reader : IModelReader
    {
        public Model3DGroup Read(int ModelIndex)
        {
            ...
            CATHODE.Models.CS2 model = Editor.resource.models.FindModelForSubmesh(Editor.resource.models.GetAtWriteIndex(ModelIndex));
            for (int i = 0; i < model.Submeshes.Count; i++)
                modelGroup.Children.Add(Editor.resource.models.GetMesh(model.Submeshes[i]));
            return modelGroup;
        }
